...
const CalorieReport = ({ dailyCalories, mealCalories, mealPlan }) => {
  ...
  return (
    <div className="report-section">
      ...
      <div className="info-box">
        <p>식사량 별 섭취 칼로리(X회 기준) : {(mealCalories ?? 0).toLocaleString()} kcal</p>
      </div>
      <div className="info-box-detail">
        <ul>
          {/* 3. ✅ [핵심] 모든 데이터 접근에 Optional Chaining(?.)과 Nullish Coalescing(??)을 적용합니다. */}
          {mealPlan.carbs && (
            <li><strong>탄수화물</strong>: {mealPlan.carbs.name} {mealPlan.carbs.amount} ({(mealPlan.carbs.kcal ?? 0)} kcal)</li>
          )}
          {mealPlan.protein && (
            <li><strong>단백질</strong>: {mealPlan.protein.name} {mealPlan.protein.amount} (≈{(mealPlan.protein.kcal ?? 0)} kcal)</li>
          )}
          {mealPlan.fat && (
            <li><strong>지방</strong>: {mealPlan.fat.name} {mealPlan.fat.amount} (≈{(mealPlan.fat.kcal ?? 0)} kcal)</li>
          )}
          {/* 커스텀 식단이 있을 경우에만 이 부분을 화면에 표시합니다. */}
          {mealPlan.custom && (
            <li><strong>커스텀</strong>: {mealPlan.custom.name} {mealPlan.custom.amount} (≈{(mealPlan.custom.kcal ?? 0)} kcal)</li>
          )}
        </ul>
